...
max_steering_angle = pi / 4.0 # keep in mind the limitations of a real car.
...
class robot:
    ...
    def move(self, motion, tolerance=0.001):
        ...
        if distance < 0.0:
            raise ValueError, 'Moving backwards is not vaild'
        ...
        if abs(turn) < tolerance:
            ...
            res.orientation = (self.orientation + turn) % (2.0 * pi)
            res.x = cx + (sin(res.orientation + turn) * radius)
            res.y = cy - (cos(res.orientation + turn) * radius)
    ...
    def sense(self, add_noise=1):
        ...
        for i in range(len(landmarks)):
            bearing = atan2(landmarks[i][0] - self.x, landmarks[i][1] - self.y) - self.orientation
