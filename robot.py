...
class plan:
    ...
    # -----
    # make_heuristic:
    #       make heuristic function for the grid
    #
    ...
    def astar(self):
        if self.heuristic == []:
            raise ValueError, "Heuristic must be defined to run A star"
...
class robot:
    ...
    # ------
    # init:
    #       create robot and initializes location/orientation to 0, 0, 0
    #
    ...
    #       every step of move, it will call move function once.
    

    def move(self, grid, steering, distance, tolerance = 0.001, max_steerin_angle = pi / 4.0):
        if steering > max_steerin_angle:
            steering = max_steerin_angle
        if steering < -max_steerin_angle:
            steering = -max_steerin_angle
        if distance < 0.0:
            distance = 0.0
...
# -----------------------------
# 
# this is the particles filter class
#
...
class particles:
    ...
    # -----
    # get_position
    #       extract position from a particle filter
    #
...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and tn < timeout:
        diff_cte = -cte
        ...
        diff_cte += cte
        steer = -params[0] * cte - params[1] * diff_cte      # PD controler
        ...
        if printflag:
            print "Pose:", myrobot, "CTE:", cte, "Step:", index, "Control:", myrobot.num_steps
